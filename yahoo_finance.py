...
#---------------These Portion will be deleted---------------------------------------------
total_time = str(time.time() - startTime)[:3]



embed = DiscordEmbed(title='Flippa For Fiverr', description= f'Finished! It took {total_time}(seconds) to scrape all.')
with open('yahoo_finance_final.csv', "rb") as f:
    webhook.add_file(file=f.read(), filename=f'yahoo_finance_final.csv')
